...
api_key = "758E4ADDD6F6D3D45F0767E90AD8E11E"


class Database:
    def __init__(self, database="steam"):
    ...
    def insert_listing(self, lst):
        ...
        # print(len(lst.steam_id))
        # print('~~~')
        if len(lst.steam_id) < 20:
            avatar = parse_hash_from_link(lst.owner_avatar)
            self.cursor.execute(
                "INSERT INTO listings(game, item_name, time, price, owner_name, owner_avatar, steam_id) "
                f"VALUES ({lst.game}, %s, now(), {lst.price}, %s, '{avatar}', '{lst.steam_id}')"
                " ON CONFLICT DO NOTHING", (lst.item_name, lst.owner_name)
            )
            if self.cursor.rowcount:
                print(f"{datetime.datetime.now()} {lst.game: <5} {lst.item_name: <60} ${lst.price: <4} {lst.owner_name} {lst.steam_id}")
